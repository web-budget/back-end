...
import br.com.webbudget.infrastructure.repository.registration.CostCenterRepository.Specifications.byFullName
...
data class CostCenterFilter(
    ...
    override fun toSpecification(): Specification<CostCenter> {
        return byActive(status?.value)
            .and(
                byName(filter)
                    .or(byFullName(filter))
                    .or(byDescription(filter))
            )
    }
}
